...
class DSCamera(object):
    ...
    def __init__(
        ...
        if json_filename != "":
            ...
            try: 
                self.f = cam_calib_data["f"]
            except Exception as e:
                print('f is not defined in calib json.',json_filename)
                self.f = None
    ...
    def to_perspective(self, img, img_size=(512, 512), f=0.5):
        if self.f is not None: f = self.f
        # Generate 3D points
        h, w = img_size
        z = f * min(img_size)
        x = np.arange(w) - w / 2
    ...
    def apply_undistort(self, img, img_size=(512, 512)):
        return self.to_perspective(img, img_size=img_size)
